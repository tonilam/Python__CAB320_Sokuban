...
##        print "Possible move: ", L
    ...
    def goal_test(self, state):
        ...
        if '@' in state:
            ...
            return False
        checkstate[i_player] = ' ';
##        if (self.step_inspected == 1558):
##            print "Now checking:"
##            self.print_state(state)
##            print "Is it equal to goal?"
##            self.print_state(self.goal)
...
##        next_state = state[:] # Note that  next_state=state   would simply create an alias
        ...
        if (self.step_inspected % 10000 == 0):
            print "Inspecting node #", self.step_inspected, " - #", self.step_inspected + 9999
            print "At node #", self.step_inspected+1, ", the state is:"
...
##        print "goalboxes = ", i_goalboxes
        ...
        while '$' in searchlist:
            ...
            i_box += 1
            searchlist = searchlist[i_next_start+1:]
##        print "normalboxes = ", i_normalboxes
        searchlist = list(self.initial)
    ...
    def print_solution(self, goal_node):
        ...
        path = goal_node.path()
        # print the solutiona
        print "Total ", self.no_of_boxes, " boxes"
        print self.step_inspected, " nodes was expanded"
...
if __name__ == "__main__":
    ...
    for i in range(61,201,2):
        puzzleFileName = "./warehouses/warehouse_"+str(i)+".txt"
        ...
        # find out the largest column size and the row size
        cols = len(visualized_by_line[0])
        rows = 0;
        for line in visualized_by_line:
            rows += 1
            if len(line) > cols:
                cols = len(line)
        ...
        for line in visualized_by_line:
            for element in range(cols):
                if line[element]:
                    problem.append(line[element])
                else:
                    problem.append(' ')
        sp = Sliding_puzzle(nr=rows, nc=cols, initial=tuple(problem), N=1000)
