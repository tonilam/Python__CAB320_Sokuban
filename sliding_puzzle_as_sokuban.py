...
class Sliding_puzzle(Problem):
    ...
    def actions(self, state):
        ...
        if (state[i_player - self.nc] != '#') or\
           ((i_box >= 0) and (state[i_player - self.nc] == state[i_box]) and (state[i_box - self.nc] != '#')):
            L.append('U')
        ...
        if (state[i_player - 1] != '#') or\
           ((i_box >= 0) and (state[i_player - 1] == state[i_box]) and (state[i_box - 1] != '#')):
            L.append('L')
...
##        return s
        ...
        print "possible actions at this state is ", possibleAction
...
if __name__ == "__main__":
    ...
    sp = Sliding_puzzle(nr=6, nc=4, N=5)
